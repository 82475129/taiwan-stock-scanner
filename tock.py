...
def run_analysis(sid, name, df, config, is_manual=False):
    ...
    try:
        ...
        # 防禦式計算平均量
        if len(df) > 1:
            v_slice = df["Volume"].iloc[-21:-1]
            v_avg = float(v_slice.mean()) if not v_slice.empty and not pd.isna(v_slice.mean()) else 0
        else:
            v_avg = 0
        v_last = df["Volume"].iloc[-1]
        if v_avg > 0 and v_last > v_avg * 1.8:
...
if st.session_state.results_data:
    ...
    t_df = pd.DataFrame([{"收藏": r["收藏"], "代碼": r["sid"], "名稱": r["名稱"],
                          "現價": r["現價"], "符合訊號": r["符合訊號"]} for r in d_data])
