...
def run_analysis(df, sid, name, config, force_show=False):
    ...
    try:
        ...
        if force_show:
            return {"sid": sid, "name": name, "price": round(c, 2), "vol": int(v_last/1000), "hits": hits if hits else ["🔍 一般觀察"], "df": df, "lines": (sh, ih, sl, il, x)}
        ...
        if config["f_ma20"] and c < m20: is_hit = False
...
# 3. 掃描執行邏輯
# ==========================================
if IS_STREAMLIT: st.title(f"📈 形態監控中心 ({app_mode})")
...
if trigger_scan:
    ...
    try:
        ...
        if status_ui: status_ui.update(label=f"✅ 完成 (顯示 {len(results)} 檔)", state="complete")
...
if IS_STREAMLIT and results:
    st.subheader("📊 總覽")
    summary_df = pd.DataFrame([{"代碼": f"https://tw.stock.yahoo.com/quote/{r['sid']}", "名稱": r["name"], "現價": r["price"], "成交量(張)": r["vol"], "狀態": " / ".join(r["hits"])} for r in results])
