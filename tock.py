...
st.markdown("""
    ...
    .price { color: #d63031; font-weight: 800; font-size: 1.25rem; }
...
with st.sidebar:
    ...
    if "今日" in selected_mode:
        ...
        run_now = st.button("🚀 執行搜尋", type="primary", use_container_width=True)
    else:
        run_now = False
...
if st.session_state.current_mode == "🌐 顯示所有股票連結":
    ...
    for item in scan_results:
        ...
        # 修正：加入千分位格式化與小數點控制
        formatted_price = f"{item['price']:,.1f}" if item['price'] >= 100 else f"{item['price']:.2f}"
